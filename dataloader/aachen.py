...
class AachenSegFull(data.Dataset):
    def __init__(self,
                 image_path,
                 label_path,
                 grgb_gid_file,
        ...
        super(AachenSegFull, self).__init__()
    ...
    def get_item_seg(self, idx):
        ...
        # print(osp.join(self.label_path, fn.replace('jpg', 'png')))
        seg_img = cv2.imread(osp.join(self.label_path, fn.replace('jpg', 'png')))
        # print(img.shape, seg_img.shape)
        seg_img = cv2.resize(seg_img, dsize=(img.shape[1], img.shape[0]), interpolation=cv2.INTER_NEAREST)
        # print(img.shape, seg_img.shape)
        # exit(0)
