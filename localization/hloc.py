...
def run_aachen(args):
    ...
    Localizer.load_db_rec(seg_dir=db_seg_dir, list_fn=db_imglist_fn, save_tmp_fn=save_tmp_fn)
...
def run_robotcar(args):
    ...
    # query_tag = "dusk"
    # query_tag = "rain"
    # query_tag = "overcast-summer"
    # query_tag = "overcast-winter"
    # query_tag = "snow"
    # query_tag = "dawn"
    # query_tag = "night"
    # query_tag = "night-rain"
    # query_tag = "full"
    # query_tag = "sdd"
    query_tag = "rssw"
    # query_tag = "swow"
    ...
    # query_tags = ['night', 'night-rain', 'rain', 'sun', 'snow', "overcast-summer", "overcast-winter"]
    # query_tags = ['snow', "overcast-winter"]
    # query_tags = ['sun', 'dusk', "dawn"]
    # query_tags = ['sun', 'dusk', "dawn", 'rain', 'snow', "overcast-summer", "overcast-winter"]
    query_tags = ['rain', 'snow', "overcast-summer", "overcast-winter"]
    # query_tags = ['night', 'night-rain']
    # query_tags = ['night-rain']
    # query_tags = ['night']

    # query_tags = [query_tag]

    # query_tag = "overcast-reference"

    # query_list = failed_cases_sun
    # query_list = vis_traj
    query_list = None
    save_fn = save_fn + query_tag
    ...
    matcher_name = args.matcher_method
    local_feat_name = args.features.as_posix().split("/")[-1].split(".")[0].split('_')[-1]

    save_fn = save_fn + "l{:d}c{:d}r{:d}{:s}{:s}{:s}fh{:.3f}{:s}".format(k_seg,
                                                                         k_can,
                                                                         k_rec,
                                                                         feat_type,
                                                                         local_feat_name,
    ...
    q_imglist_fn = "datasets/robotcar/robotcar_queries_with_intrinsics_rear.txt"
    q_imglist = []
    with open(q_imglist_fn, "r") as f:
        lines = f.readlines()
        for l in lines:
    ...
    for qname, qinfo in tqdm(queries):
        if qname.split('/')[0] not in query_tags:
            continue

        if args.only_gt > 0:
            if qname not in gt_poses.keys():
                continue
        ...
        qvec, tvec, n_inliers, logs = pose_from_cluster_with_matcher(qname, qinfo, db_ids, db_images,
                                                                     ...
                                                                     inlier_ths=inlier_ths,
                                                                     retrieval_sources=retrieval_sources,
                                                                     gt_qvec=gt_poses[qname][
                                                                         'qvec'] if qname in gt_poses.keys() else None,
                                                                     gt_tvec=gt_poses[qname][
                                                                         'tvec'] if qname in gt_poses.keys() else None,
                                                                     )
        all_logs.append(logs)
        ...
        print_text = "All {:d}/{:d}/{:d} failed/hard cases top@1:{:.2f}, rec:{:.2f} nv:{:.2f}, {:d}".format(
            n_failed_hard,
            n_failed, n_total,
            n_top1 / n_total,
            n_rec / n_total,
    ...
    with open(results, 'w') as f:
        for q in poses:
            ...
            name = q.split('/')[-1]
            camera = q.split('/')[-2]
            f.write(f'{camera}/{name} {qvec} {tvec}\n')
