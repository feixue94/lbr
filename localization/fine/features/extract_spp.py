...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_path", type=str, default="/home/SENSETIME/xuefei/Dataset/aachen/output/det",
                        help='path to save descriptors')
    parser.add_argument("--image_list_file", type=str, default="img_list_hpatches_list.txt",
                        help='path to save descriptors')

    spp = SuperPointNet()
    spp.load_state_dict(torch.load('./models/superpoint_v1.pth'))
    spp = spp.cuda().eval()
    ...
    for line in tqdm(lines, total=len(lines)):
        ...
        with torch.no_grad():
            ...
            img = torch.from_numpy(img)
            img = img.view(1, 1, img.size(0), img.size(1)).cuda()
            keypoints, scores, descriptors = extract_spp(model=spp, img=img, need_desc=True)

            keypoints = np.transpose(keypoints, [1, 0])
            descriptors = np.transpose(descriptors, [1, 0])
            scores = keypoints[:, 2]

            print("pts: ", keypoints.shape[0])
            print("desc: ", descriptors.shape)
            print("score: ", scores.shape)

            plot_keypoint(img_path=img_path, pts=keypoints, scores=scores)
            splits = line.strip().split('/')
            # print(splits)
            # exit(0)
            # with open(osp.join(output_dir, splits[-2] + "-" + splits[-1] + '.spp'), 'wb') as output_file:
            #     # with open(osp.join(img_path + '.spp'), 'wb') as output_file:
            #     np.savez(
            #         output_file,
            #         keypoints=keypoints,
            #         scores=scores,
